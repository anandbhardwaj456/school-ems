...
export default function FeesScreen() {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={fees}
        keyExtractor={(item) => item.invoiceId?.toString() || item.id?.toString()}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card}>
            <Text style={styles.cardTitle}>{item.categoryName || 'Fee Invoice'}</Text>
            <Text style={styles.cardText}>Amount: ₹{item.amount || 0}</Text>
            <Text style={[styles.cardText, { color: item.status === 'paid' ? '#16a34a' : '#dc2626' }]}>
