...
export default function LibraryScreen() {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={books}
        keyExtractor={(item) => item.bookId?.toString() || item.id?.toString()}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card}>
            ...
            <Text style={styles.cardText}>Author: {item.author || 'N/A'}</Text>
            <Text style={styles.cardText}>ISBN: {item.isbn || 'N/A'}</Text>
            <Text style={[styles.cardText, { color: item.available ? '#16a34a' : '#dc2626' }]}>
